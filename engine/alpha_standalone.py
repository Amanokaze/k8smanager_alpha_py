...
DEFAULT_INFO = {
    ...
    "KUBE_API_KEY": None,
    "KUBE_MGR_ST_INTERVAL": 5,
    "KUBE_MGR_LT_INTERVAL": 30,
...
KUBE_MGR_LT_INTERVAL = DEFAULT_INFO["KUBE_MGR_LT_INTERVAL"]
...
# Input data to Tableinfo
def input_tableinfo(name, cursor, conn):
    cursor.execute(stmt.SELECT_TABLEINFO_TABLENAME.format(name))
    result = cursor.fetchone()
    exec_stmt = stmt.UPDATE_TABLEINFO.format(name) if result[0] == 1 else stmt.INSERT_TABLEINFO.format(name)
    cursor.execute(exec_stmt)
    conn.commit()
    log.write("PUT", f"{name} data is added in kubetableinfo table.")
...
def get_ontunetime(cursor):
    cursor.execute(stmt.SELECT_ONTUNEINFO)
    result = cursor.fetchone()
    return result[0] if result else 0

def get_agenttime(cursor):
    ...
    try:
        ...
        for node in nodes.items:
            ...
            node_data[nodename] = {
                ...
                "ephemeralstorage": change_quantity_unit(node.status.capacity["ephemeral-storage"]),
                "memorysize": change_quantity_unit(node.status.capacity["memory"]),
        ...
        log.write("GET", "Kube Node Data Import is completed.")
...
def check_ontune_schema():
    ...
    with db.get_resource_rdb() as (cursor, _, conn):
        ...
        for obj_name in schema_object["reference"]:
            ...
            if result[0] == 1:
                log.write("GET", f"Reference table {obj_name} is checked.")
            else:
                log.write("GET", f"Reference table {obj_name} doesn't exist. now it will be created.")
                ...
                conn.commit()
                log.write("PUT", f"Reference table {obj_name} creation is completed.")

                input_tableinfo(obj_name, cursor, conn)
        ...
        for obj_name in schema_object["metric"]:
            ...
            if result[0] == 2:
                log.write("GET", f"Realtime/avg{obj_name}{table_postfix} metric tables are checked.")
            else:
                # Metric Table Creation
                log.write("GET", f"Realtime/avg{obj_name}{table_postfix} metric tables doesn't exist. now they will be created.")
                creation_prefix = "create table if not exists"
                column_properties = ",".join(list(" ".join(x) for x in properties))
                ...
                for table_prefix in ('realtime','avg'):
                    ...
                    cursor.execute(table_creation_statement)
                    conn.commit()
                    log.write("PUT", f"Metric table {full_table_name} creation is completed.")
                    ...
                    conn.commit()
                    log.write("PUT", f"Metric table index i{full_table_name} creation is completed.")

                    input_tableinfo(obj_name, cursor, conn)

    return schema_object

def put_kube_data_to_ontune(info, kube_data, schema_obj):
    # Put data
    with db.get_resource_rdb() as (cursor, cur_dict, conn):
        # Kube Data variables
        ...
        try:
            ...
            column_data = insert_columns_ref(schema_obj, "kubemanagerinfo")
            value_data = insert_values([info["manager_name"], info["manager_name"], info["manager_ip"]])
            cursor.execute(stmt.INSERT_TABLE.format("kubemanagerinfo", column_data, value_data))
            conn.commit()
            #log.write("PUT", f"Kubemanagerinfo insertion is completed - {info['manager_ip']}")
        ...
        if not manager_id:
            #log.write("GET", "Kubemanagerinfo has an error. Put data process is stopped.")
            return False
        ...
        try:
            ...
            column_data = insert_columns_ref(schema_obj, "kubeclusterinfo")
            value_data = insert_values([manager_id, info["cluster_name"], info["cluster_name"], info["cluster_address"]])
            cursor.execute(stmt.INSERT_TABLE.format("kubeclusterinfo", column_data, value_data))
            conn.commit()
            #log.write("PUT", f"Kubeclusterinfo insertion is completed - {info['cluster_address']}")
        ...
        if not cluster_id:
            #log.write("GET", "Kubeclusterinfo has an error. Put data process is stopped.")
            return False
        ...
        try:
            ...
            for new_ns in new_namespace_list:
                column_data = insert_columns_ref(schema_obj, "kubensinfo")
                value_data = insert_values([cluster_id, new_ns, 1])
                cursor.execute(stmt.INSERT_TABLE.format("kubensinfo", column_data, value_data))
                conn.commit()
                log.write("PUT", f"Kubensinfo insertion is completed - {new_ns}")
            ...
            if len(old_ns_id_list) > 0:
                ...
                conn.commit()
                log.write("PUT", f"Kubensinfo enabled state is updated - {','.join(old_ns_id_list)}")
        except Exception as e:
            conn.rollback()
            log.write("GET", f"Kubenamespaceinfo has an error. Put data process is stopped. - {str(e)}")
            return False
        ...
        try:
            for node in node_list:
                if node in node_query_dict:
                    if node_list[node]["uid"] != node_query_dict[node]["_nodeuid"]:
                        update_data = update_values(schema_obj, "kubenodeinfo", node_list[node])
                        cursor.execute(stmt.UPDATE_NODEINFO.format(update_data, node_query_dict[node]["_nodeid"]))
                        conn.commit()
                        log.write("PUT", f"Kubenodeinfo information is updated - {node}")

                else:
                    column_data = insert_columns_ref(schema_obj, "kubenodeinfo")
                    value_data = insert_values([manager_id, cluster_id] + list(node_list[node].values()))
                    cursor.execute(stmt.INSERT_TABLE.format("kubenodeinfo", column_data, value_data))
                    conn.commit()
                    log.write("PUT", f"Kubenodeinfo insertion is completed - {node}")
            ...
            if len(old_node_id_list) > 0:
                ...
                conn.commit()
                log.write("PUT", f"Kubensinfo enabled state is updated - {','.join(old_node_id_list)}")
            ...
            conn.rollback()
            log.write("GET", f"Kubenodeinfo has an error. Put data process is stopped. - {str(e)}")
            return False

        if not node_query_dict:
            log.write("GET", "Kubenodeinfo is empty. Put data process is stopped.")
        ...
        try:
            ...
            for sysco in new_node_sysco_list:
                ...
                column_data = insert_columns_ref(schema_obj, "kubenodesyscoinfo")
                value_data = insert_values([nodeid, sysco["containername"], 1])
                cursor.execute(stmt.INSERT_TABLE.format("kubenodesyscoinfo", column_data, value_data))
                conn.commit()
                log.write("PUT", f"Kubenodesyscoinfo insertion is completed - {sysco['nodename']} / {sysco['containername']}")
            ...
            if old_node_sysco_list:
                ...
                if len(old_node_sysco_id_list) > 0:
                    ...
                    conn.commit()
                    log.write("PUT", f"Kubenodesyscoinfo enabled state is updated - {','.join(old_node_id_list)}")
            ...
            conn.rollback()
            log.write("GET", f"Kubenodesystemcontainer info has an error. Put data process is stopped. - {str(e)}")
            return False
        ...
        try:
            ...
            for pod in new_pod_list:
                ...
                column_data = insert_columns_ref(schema_obj, "kubepodinfo")
                value_data = insert_values([nodeid, nsid, pod["uid"], pod["name"], 1])
                cursor.execute(stmt.INSERT_TABLE.format("kubepodinfo", column_data, value_data))
                conn.commit()
                log.write("PUT", f"Kubepodinfo insertion is completed - {pod['node']}/{pod['name']}")

            # Old Pod Update
            if len(old_pod_id_list) > 0:
                cursor.execute(stmt.UPDATE_ENABLED.format("kubepodinfo", "_podid", ",".join(old_pod_id_list)))
                conn.commit()
                log.write("PUT", f"Kubepodinfo enabled state is updated - {','.join(old_pod_id_list)}")
            ...
            conn.rollback()
            log.write("GET", f"Kubepodinfo has an error. Put data process is stopped. - {str(e)}")
            return False
        ...
        if podinfo_dict:
            ...
            try:
                ...
                for container in new_container_list:
                    ...
                    column_data = insert_columns_ref(schema_obj, "kubecontainerinfo")
                    value_data = insert_values([podid, container["name"], svtime_to_timestampz(container["starttime"])])
                    cursor.execute(stmt.INSERT_TABLE.format("kubecontainerinfo", column_data, value_data))
                    conn.commit()
                    log.write("PUT", f"Kubecontainerinfo insertion is completed - {podid} / {container['name']}")
                ...
                conn.rollback()
                log.write("GET", f"Kubecontainerinfo has an error. Put data process is stopped. - {str(e)}")
                return False
            ...
            try:
                ...
                for devtype in device_type_set:
                    for devinfo in new_dev_info_set[devtype]:
                        column_data = insert_columns_ref(schema_obj, "kubepoddeviceinfo")
                        value_data = insert_values([devinfo, devtype])
                        cursor.execute(stmt.INSERT_TABLE.format("kubepoddeviceinfo", column_data, value_data))
                        conn.commit()
                        log.write("PUT", f"Kubepoddeviceinfo insertion is completed - {devtype} / {devinfo}")
                ...
                conn.rollback()
                log.write("GET", f"Kubedeviceinfo has an error. Put data process is stopped. - {str(e)}")
                return False
        ...
        try:
            for node in node_query_dict:
                ...
                network_prev_cum_usage = system_var.get_network_metric("lastrealtimeperf", node)                
                ...
                node_perf = [
                    ...
                    get_ontunetime(cursor),
                    calculate('cpu_usage_percent', [node_data["cpu"]["usageNanoCores"], node_list[node]["cpucount"]]),
                    calculate('memory_used_percent', node_data["memory"]),
                    calculate('memory_swap_percent', node_data["memory"]),
                    calculate('memory_size', node_data["memory"]),
                    node_data["memory"]["rssBytes"],
                    calculate('network', [network_usage]),
                    calculate('fs_usage_percent', node_data["fs"]),
                    calculate('fs_total_size', node_data["fs"]),
                    calculate('fs_inode_usage_percent', node_data["fs"]),
                    calculate('fs_usage_percent', node_data["runtime"]["imageFs"]),
                    node_data["rlimit"]["curproc"]
                ]

                system_var.set_network_metric("lastrealtimeperf", node, network_cum_usage)

                column_data = insert_columns_metric(schema_obj, "kubelastrealtimeperf")
                value_data = insert_values(node_perf)
                ...
                conn.commit()
                log.write("PUT", f"Kubelastrealtimeperf update is completed - {node_query_dict[node]['_nodeid']}")
        except Exception as e:
            conn.rollback()
            log.write("GET", f"Kubelastrealtimeperf has an error. Put data process is stopped. - {str(e)}")
            return False

        # Update Realtime table
        ontunetime = get_ontunetime(cursor)
        agenttime = get_agenttime(cursor)
        ...
        try:
            ...
            for node in node_query_dict:
                ...
                network_prev_cum_usage = system_var.get_network_metric("nodeperf", node)
                network_cum_usage = [
                    calculate('network', [sum(list(x["rxBytes"]+x["txBytes"] for x in node_data["network"]["interfaces"]))]),
                    calculate('network', [sum(list(x["rxBytes"] for x in node_data["network"]["interfaces"]))]),
                    calculate('network', [sum(list(x["txBytes"] for x in node_data["network"]["interfaces"]))]),
                ...
                realtime_nodeperf = [
                    ...
                    calculate('fs_usage_percent', node_data["fs"]),
                    calculate('fs_total_size', node_data["fs"]),
                    calculate('fs_free_size', node_data["fs"]),
                    calculate('fs_inode_usage_percent', node_data["fs"]),
                    calculate('fs_inode_total_size', node_data["fs"]),
                    calculate('fs_inode_free_size', node_data["fs"]),
                    calculate('fs_usage_percent', node_data["runtime"]["imageFs"]),
                    node_data["rlimit"]["maxpid"],
                    node_data["rlimit"]["curproc"]
                ]

                system_var.set_network_metric("nodeperf", node, network_cum_usage)

                table_name = f"realtimekubenodeperf{table_postfix}"
                column_data = insert_columns_metric(schema_obj, "kubenodeperf")
                value_data = insert_values(realtime_nodeperf)

                cursor.execute(stmt.INSERT_TABLE.format(table_name, column_data, value_data))
                conn.commit()
                log.write("PUT", f"{table_name} update is completed - {node_query_dict[node]['_nodeid']}")
                ...
                for sysco_query_data in node_sysco_query_dict[nodeid]:
                    ...
                    realtime_node_sysco = [
                        ...
                        ontunetime,
                        agenttime,
                        calculate('cpu_usage_percent', [sysco_data[containername]["cpu"]["usageNanoCores"], node_list[node]["cpucount"]]) if "usageNanoCores" in sysco_data[containername]["cpu"] else 0,
                        calculate('memory_used_percent', sysco_data[containername]["memory"]),
                        calculate('memory_swap_percent', sysco_data[containername]["memory"]),
                        calculate('memory_size', sysco_data[containername]["memory"]),
                    ...
                    column_data = insert_columns_metric(schema_obj, "kubenodesysco")
                    value_data = insert_values(realtime_node_sysco)
                ...
                for pod in pod_data:
                    ...
                    network_prev_cum_usage = system_var.get_network_metric("podperf", podid)
                    network_cum_usage = [
                        calculate('network', [sum(list(x["rxBytes"]+x["txBytes"] for x in pod["network"]["interfaces"]))]) if "network" in pod and "interfaces" in pod["network"] else 0,
                        calculate('network', [sum(list(x["rxBytes"] for x in pod["network"]["interfaces"]))]) if "network" in pod and "interfaces" in pod["network"] else 0,
                        calculate('network', [sum(list(x["txBytes"] for x in pod["network"]["interfaces"]))]) if "network" in pod and "interfaces" in pod["network"] else 0,
                    ...
                    realtime_podperf = [
                        ...
                        ontunetime,
                        agenttime,
                        calculate('cpu_usage_percent', [pod["cpu"]["usageNanoCores"], node_list[node]["cpucount"]]) if "cpu" in pod and "usageNanoCores" in pod["cpu"] else 0,
                        calculate('memory_used_percent', pod["memory"]),
                        calculate('memory_swap_percent', pod["memory"]),
                        calculate('memory_size', pod["memory"]),
                        ...
                        sum(int(x["inodesUsed"]) for x in pod["volume"]) if "volume" in pod and "usedBytes" in pod["volume"][0] else 0,
                    ...
                    system_var.set_network_metric("podperf", podid, network_cum_usage)

                    table_name = f"realtimekubepodperf{table_postfix}"
                    column_data = insert_columns_metric(schema_obj, "kubepodperf")
                    value_data = insert_values(realtime_podperf)

                    cursor.execute(stmt.INSERT_TABLE.format(table_name, column_data, value_data))
                    conn.commit()
                    log.write("PUT", f"{table_name} update is completed - {uid}")
                    ...
                    for pod_container in pod["containers"]:
                        realtime_containerperf = [
                            ...
                            ontunetime,
                            agenttime,
                            calculate('cpu_usage_percent', [pod_container["cpu"]["usageNanoCores"], node_list[node]["cpucount"]]) if "cpu" in pod_container and "usageNanoCores" in pod_container["cpu"] else 0,
                            calculate('memory_used_percent', pod_container["memory"]),
                            calculate('memory_swap_percent', pod_container["memory"]),
                            calculate('memory_size', pod_container["memory"]),
                        ...
                        column_data = insert_columns_metric(schema_obj, "kubecontainerperf")
                        value_data = insert_values(realtime_containerperf)
                    ...
                    if "network" in pod and "interfaces" in pod["network"]:
                        for pod_network in pod["network"]["interfaces"]:
                            ...
                            network_prev_cum_usage = system_var.get_network_metric("podnet", podnet_key)
                            network_cum_usage = [
                                calculate('network', [pod_network["rxBytes"] + pod_network["txBytes"]]),
                                calculate('network', [pod_network["rxBytes"]]),
                                calculate('network', [pod_network["txBytes"]]),
                            ...
                            system_var.set_network_metric("podnet", podnet_key, network_cum_usage)

                            table_name = f"realtimekubepodnet{table_postfix}"
                            column_data = insert_columns_metric(schema_obj, "kubepodnet")
                            value_data = insert_values(realtime_podnet)
                    ...
                    if "volume" in pod:
                        for pod_volume in pod["volume"]:
                            ...
                            column_data = insert_columns_metric(schema_obj, "kubepodvol")
                            value_data = insert_values(realtime_podvol)
            ...
            conn.rollback()
            log.write("GET", f"Kube realtime tables have an error. Put data process is stopped. - {str(e)}")
            return False
        ...
        def insert_average_table(table_midfix, key_columns):
            try:
                ...
                select_clause = select_average_columns(schema_obj, table_midfix, key_columns)
                ...
                for row in result:
                    column_data = insert_columns_metric(schema_obj, table_midfix)
                    value_data = insert_values(list(row[:len(key_columns)]) + [ontunetime, agenttime] + list(row[len(key_columns):]))
                    
                    cursor.execute(stmt.INSERT_TABLE.format(table_lt_name, column_data, value_data))
                    conn.commit()

                log.write("PUT", f"{table_lt_name} update is completed - {ontunetime}")
            except Exception as e:
                conn.rollback()
                log.write("GET", f"Kube average tables have an error. Put data process is stopped. - {str(e)}")

        if system_var.get_duration() % KUBE_MGR_LT_INTERVAL == 0:
...
def thread_func(kube_cfg):
    # 기본 구조는 KUBE_MGR_ST_INTERVAL 간격으로 Thread를 생성해서 기능을 수행하도록 함
    threading.Timer(KUBE_MGR_ST_INTERVAL, thread_func, kwargs={"kube_cfg":kube_cfg}).start()
    system_var.refresh_duration()
    
    # API 기본 정보를 가져오는 부분
    # Cluster name 정보는 API를 통해서 가져올 수 없는 것으로 최종 확인되었음
    api_version_info = client.CoreApi(client.ApiClient(kube_cfg)).get_api_versions()
    kube_basic_info = {
        "manager_name": db.get_basic_info("managername"),
    ...
    api = client.CoreV1Api(client.ApiClient(kube_cfg))
    (ndata, nmdata, pmdata) = get_kube_node_data(api)
    kube_data = {
        "node": ndata,
        "node_metric": nmdata,
        "pod_metric": pmdata,
        "ns": get_kube_ns_data(api)
    }
    ...
    schema_obj = check_ontune_schema()
    result = put_kube_data_to_ontune(kube_basic_info, kube_data, schema_obj)
...
def main():
    ...
    kube_cfg.api_key['authorization'] = KUBE_API_KEY
    kube_cfg.api_key_prefix['authorization'] = 'Bearer'
    kube_cfg.host = KUBE_HOST
    kube_cfg.verify_ssl = True
    kube_cfg.ssl_ca_cert = 'ca.crt'

    # onTune DB Connection을 위한 Connection Pool 역시 사전에 생성토록 함
    # Connection Pool이 정상 생성될 경우에만 최초 Thread를 가동토록 할 것
    try:
        db.create_connection()
        atexit.register(db.shutdown_connection_pool)
        threading.Thread(target=thread_func, kwargs={"kube_cfg":kube_cfg}).start()
